...
将 5 通道展开剂比例参数翻译为 SY-03B (T-06 阀头) 注射泵 DT 协议指令。
...
#  常量
# ---------------------------------------------------------------------------
T06_SOLVENT_PORTS = 5     # T-06 阀头溶剂入口数（端口 1-5）
...
def translate_develop_cmd(
    ...
    *,
    ...
    Args:
        ratios: 5 个溶剂通道的比例值，对应物理端口 2-6。
                例如 [3, 2, 1, 0, 0] 表示 端口2:端口3:端口4 = 3:2:1。
                值为 0 表示该通道不参与混合。
    ...
    for i, ratio in enumerate(ratios, start=2):
        ...
        command = f"I{i}A{cum_steps}M{step_delay}"
...
def build_forward_instructions(
    ...
    *,
    ...
    """
    cmd_seq = translate_develop_cmd(
        ratios, total_volume_ml,
        pump_addr=pump_addr,
    ...
    # 分离吸液子指令和打液子指令
    asp_cmds = [c for c in cmd_seq if c["channel"] != output_port]
    disp_cmd = next(c for c in cmd_seq if c["channel"] == output_port)

    # 组装复合指令：/addr + Vasp_speed + 吸液子指令 + Vdisp_speed + 打液子指令 + R
    parts = [f"/{pump_addr}V{asp_speed}"]
    parts.extend(c["command"] for c in asp_cmds)
    parts.append(f"V{disp_speed}")
    parts.append(disp_cmd["command"])
    parts.append("R")
...
def translate_develop_params(
    ...
    Args:
        ratios: 5 个溶剂通道比例值（UI 输入，不直接写 PLC）
        total_volume_ml: 展开剂总体积 (mL)
