...
class ThreadsafePlcCameraHost:
    ...
    def rail_move(self, position_code: int) -> object:
        """命令 PLC 把地轨 (地轨轴11Y) 移到指定工位位置并阻塞至到位。

        参数:
            position_code: 地轨绝对位置码 (Rail 原子动作码, legacy position[1..6])

        返回:
            dict, 含动作名/站名/位置码与 PLC L2 原子动作结果
        """
        if self.plc_action_client is None:
            raise RuntimeError("机器人地轨原子动作需要 PLC L2 client")
        result = self._submit(
            self.plc_action_client.execute(
                "rail",
                position_code,
