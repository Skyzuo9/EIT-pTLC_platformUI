...
from .robot_service import RobotActionService
...
class RouteExecutor:
    ...
    def execute(self, plan: ResolvedRoutePlan) -> RouteExecutionResult:
        ...
        # 地轨移动会带动机械臂, 必须与机器人步骤同处独占锁内, 故一并计入边界
        robot_indexes = [
            index for index, step in enumerate(plan.steps)
            if isinstance(step, (RobotRouteStep, RailMoveStep))
        ]
    ...
    def _execute_step(
        ...
        if isinstance(step, RailMoveStep):
            return self.host.rail_move(step.position_code)
