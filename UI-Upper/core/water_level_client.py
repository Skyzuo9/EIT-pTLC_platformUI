"""
WaterLevelClient — 液位检测模块 MQTT 客户端

与 PLCClient 模式对齐：单连接、自动重连、回调机制。
运行在 asyncio 事件循环中，通过 paho-mqtt 后台线程 + call_soon_threadsafe 桥接。

职责:
  - 订阅上行主题: water_level/data, /status, /alarm, /param/detect, /ack
  - 暴露 async send_command(cmd, payload) 统一发送接口
...
# 全局弱引用：供 atexit 回调做最后清理（进程退出前 force_cleanup）
_instances: list = []
...
class WaterLevelClient:
    ...
    def __init__(self, broker_ip: str, broker_port: int = 1883,
        ...
        self._force_cleaned = False  # force_cleanup 幂等标志
        ...
        # paho 客户端 (在 connect() 中创建)
        self._client: mqtt.Client | None = None
        self._loop = None

        # 注册 atexit 清理（进程退出前最后手段）
    ...
    def _dispatch(self, callbacks: list[Callback], *args):
        ...
        for cb in callbacks:
            loop.call_soon_threadsafe(
                lambda c=cb, a=args: self._safe_call(c, *a))
    ...
    def _on_disconnect(self, client, userdata, flags, rc, properties=None):
        ...
        # 香橙派 LWT: 断开后如果 device_online 仍为 True，标记为 offline
    ...
    async def disconnect(self):
        ...
        if self._client:
            ...
            # loop_stop() 可能阻塞（等待后台线程结束），加 2 秒超时
            loop_stop_done = await asyncio.get_running_loop().run_in_executor(
                None, self._timed_loop_stop, 2.0
            )
    ...
    def force_cleanup(self) -> None:
        """同步强制清理（在 os._exit 前调用）。幂等，可重复调用。

        直接关闭 paho 客户端 socket + 停止后台线程，
        不等待任何 MQTT 消息完成。
        """
    ...
    async def send_command(self, cmd: str, payload: dict | None = None) -> bool:
        ...
        Args:
            cmd: 命令名 (如 "stream_start", "set_detect_param")
            payload: 命令参数 (dict)
        ...
        if not self.is_connected or not self._client:
            log.warning("[WL] 未连接(is_connected=%s), 无法发送命令: %s", self.is_connected, cmd)
            return False
        ...
        try:
            ...
            log.error("[WL TX] %s 异常: %s", cmd, e)
            return False

    # ---- 属性 ----
    @property
    def is_connected(self) -> bool:
        return self.connected and self.device_online
