...
def _local_native_dir():
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), "native", "win64")
...
def _add_dll_search_dirs(paths):
    for path in paths:
...
def _load_win_dll(name):
    native_dir = _local_native_dir()
    local_api_dir = os.path.join(native_dir, "apidll")
    local_genicam_dir = os.path.join(native_dir, "genicam")
    local_gentl_dir = os.path.join(native_dir, "gentl")

    dll_dirs = _existing_dirs(local_genicam_dir, local_api_dir, local_gentl_dir)
    _add_dll_search_dirs(dll_dirs)
    if os.path.isdir(local_gentl_dir):
        os.environ.setdefault("GENICAM_GENTL64_PATH", local_gentl_dir)

    local_dll = os.path.join(local_api_dir, name)
    dll_name = local_dll if os.path.isfile(local_dll) else name
    try:
        ...
        raise OSError(
            "%s not found. Expected vendored Daheng runtime under %s."
            % (name, native_dir)
        ) from exc
...
if sys.platform == 'linux2' or sys.platform == 'linux':
    ...
    try:
        ...
        raise OSError('Cannot find libdximageproc.so or libgxiapi.so.') from exc
else:
    dll = _load_win_dll('DxImageProc.dll')
