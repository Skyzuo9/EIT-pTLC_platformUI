#!/usr/bin/env python3
"""展缸/下料点位吸附基准预置横向分量符号取反脚本 (一次性数据迁移 v4, 入库留审计).

功能:
    纠正 v3 横向分量的符号。rotary-down 是 180° 翻转(吸盘从托板朝上翻成持板朝下),
    180° 翻转会把基准差向量**镜像**: 沿翻转轴的分量(≈21, 插入方向, v2 真机已验证)
    保持不变, 垂直分量(≈6.8, 横向)**变号**。v3 按不镜像处理给了 +6.8, 真机实测
    "同一侧、量级约翻倍"(7mm -> 14mm)精确证实真值为 -6.8。
    正确映射 = 先镜像(横向分量取反)再按 Rz(点rz - P64rz) 旋转:
        塔A P11-P14: (-6.80, -21.00)   塔B P15-P18: (+6.85, +20.98)   下料 P22: (+20.96, -6.92)

做法:
    从原始示教值锚定重算: 新 xy = 原始 xy + 镜像映射向量, z = 原始示教值。
    镜像实现: 先算 mapped = Rz(dyaw)·v, 再对各工位沿其插入轴做镜像
    (缸塔插入轴 = Y, 镜像即 x 取反; 下料插入轴 = X, 镜像即 y 取反)。
    rx/ry/rz 与 joint 原样保留, 不盖 calibrated_at。

防重跑守卫:
    执行前逐点核对当前 xyz 与 post-v3 快照一致, 不符即拒绝写盘并退出非 0。

用法:
    python -m eit_ptlc.tools.preset_basis_shift_v4 --dry-run   # 只打印 老pose -> 新pose
    python -m eit_ptlc.tools.preset_basis_shift_v4             # 全量校验通过后落盘
...
# post-v3 状态快照 (x, y, z): 防重跑锚
_EXPECTED_POST_V3_XYZ = {
    "P11": (-569.338701, -215.473809, -56.330429),
    "P12": (-568.338701, -214.673806, 125.391556),
    "P13": (-567.738725, -213.673806, 304.791565),
    "P14": (-566.138505, -213.073601, 484.791748),
    "P15": (-599.823822, 212.167007, -55.533272),
    "P16": (-598.323698, 211.866928, 124.466713),
    "P17": (-597.823698, 211.367111, 303.466675),
    "P18": (-596.323698, 210.667114, 484.966675),
    "P22": (585.885037, -416.419679, -92.365082),
}
# 原始示教值 (v1 执行前) —— 锚定基准, 新值 = 原始 xy + 镜像映射向量, z = 原始 z
_ORIGINAL_XYZ = {
...
# 各工位插入轴: 镜像 = 垂直于插入轴的分量取反 ("y" 表示插入轴为 Y, x 取反)
_INSERT_AXIS = {
    "P11": "y", "P12": "y", "P13": "y", "P14": "y",
    "P15": "y", "P16": "y", "P17": "y", "P18": "y",
    "P22": "x",
}
# 镜像后分量符号粗核对: 塔A (-,-), 塔B (+,+), 下料 (+,-)
...
logger = logging.getLogger("preset_basis_shift_v4")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="展缸/下料点位预置横向分量符号取反 (一次性 v4)")
    ...
    # 防重跑守卫: 全部目标点当前 xyz 必须等于 post-v3 快照
    errors: list[str] = []
    for name, expected_xyz in _EXPECTED_POST_V3_XYZ.items():
        try:
        ...
        for idx, expected in enumerate(expected_xyz):
            if abs(float(pose[idx]) - expected) > 1e-6:
                errors.append(
                    f"{name} 当前 {'xyz'[idx]}={pose[idx]} 与 post-v3 快照 {expected} 不符 "
                    "(疑似已纠正过或点表已变更)")
    if len(errors) > 0:
        for line in errors:
            logger.error(line)
    ...
    # 逐点: mapped = Rz(点rz - P64rz)·v, 再沿插入轴镜像(垂直分量取反); 新 xy = 原始 xy + 镜像后向量
    for name, original in _ORIGINAL_XYZ.items():
        ...
        logger.info(
            "%s: dyaw=%.3f deg, 镜像映射=(%.4f, %.4f) | (%.6f, %.6f, %.6f) -> (%.6f, %.6f, %.6f)",
