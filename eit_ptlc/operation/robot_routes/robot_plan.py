...
from .robot_flow import MoveStep, RobotFlow, SetMountedToolStep, ToolStep, WaitStep
from eit_ptlc.driver.robot_transport import MotionProfile, TOOL_ACTION_NAMES, ToolAction
...
@dataclass(frozen=True)
class TemplateSetMountedToolStep:
    """声明权威工具态: tool_id 显式给定 (如卸刀=0), 或 from_selector 取本分支主选择值 (挂刀)."""
    step_id: str
    tool_id: int | None
    from_selector: bool
...
TemplateStep = (
    ...
    | TemplateToolStep
    | TemplateWaitStep
    | TemplateExtensionStep
    | TemplateSetMountedToolStep
)
...
class RobotPlanCatalog:
    ...
    def selector_params(
        self, template_id: str, *, allow_test: bool = False
    ) -> tuple[str, str | None]:
        """返回模板的 (主选择参数名, 次选择参数名|None).

        参数:
            template_id: 模板 id; allow_test: 是否允许测试模板
        返回:
            (selector_param, secondary_param) 供上位机按值构造 business_params
        """
        template = self._get_template(template_id, allow_test=allow_test)
        return template.selector_param, template.secondary_param
    ...
    def resolve(
        ...
        *,
        ...
        concrete = self._resolve_steps(template, point_set, primary)
    ...
    def _resolve_steps(
        ...
        point_set: PointSet,
        primary: object,
    ) -> tuple[MoveStep | ToolStep | WaitStep | SetMountedToolStep, ...]:
        result: list[MoveStep | ToolStep | WaitStep | SetMountedToolStep] = []
        seen: set[str] = set()
        ...
        def append_steps(steps: Iterable[TemplateStep]) -> None:
            for step in steps:
                ...
                elif isinstance(step, TemplateSetMountedToolStep):
                    if step.from_selector:
                        try:
                            tool_id = int(primary)
                        except (TypeError, ValueError) as exc:
                            raise RobotPlanError(
                                f"{template.template_id}/{step.step_id} from_selector "
                                f"requires integer selector, got {primary!r}"
                            ) from exc
                    else:
                        tool_id = int(step.tool_id)
                    result.append(SetMountedToolStep(step.step_id, tool_id))
                else:
    ...
    def _load_steps(
        ...
        *,
        ...
        for item in raw:
            ...
            elif step_type == "set_mounted_tool":
                from_selector = bool(item.get("from_selector", False))
                raw_tool = item.get("tool_id")
                if from_selector == (raw_tool is not None):
                    raise RobotPlanError(
                        f"{owner}/{step_id} set_mounted_tool needs exactly one of "
                        "tool_id or from_selector:true"
                    )
                tool_id = None if from_selector else int(raw_tool)
                if tool_id is not None and tool_id not in (0, 1, 2, 3):
                    raise RobotPlanError(f"{owner}/{step_id} tool_id must be 0..3")
                result.append(
                    TemplateSetMountedToolStep(step_id, tool_id, from_selector)
                )
            else:
                raise RobotPlanError(
