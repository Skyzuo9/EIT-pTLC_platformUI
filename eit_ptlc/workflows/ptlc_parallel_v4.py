...
@workflow(
    workflow_uuid="1a424e61-d0fe-5489-86ae-11ca393d21b8",
...
def ptlc_parallel_v4(
    *,
    ...
    # unilab:node_uuid=1ea72134-6541-5cf7-b84c-2bc33e8f50e4
    ...
    # unilab:node_uuid=d1e46485-4b16-52c8-b118-040ba0e0f3aa
    ...
    # unilab:node_uuid=3cb3e724-b762-56f2-beed-2fb278955a68
    ...
    # unilab:node_uuid=2b25206b-1c2d-5fae-991f-0569ea820262
    ...
    # unilab:node_uuid=6a906715-05b1-5586-beec-f61e4d9bfb5f
    with group(name="af0 开工预检"):
        # unilab:node_uuid=182a59bc-eb99-578f-8ce4-ad1ef4973726
    ...
    # unilab:node_uuid=6c1052c5-3039-595a-85c4-d28af8cd6a46
    with group(name="s1 上样上料"):
        # Exact PlatformUI station preparation; its root has no robot/rail.
        # unilab:node_uuid=338199c1-13ca-5588-b502-fce1d6756a45
        sampling_ready = sampling.sampling_prepare()  # noqa: F841
        # unilab:node_uuid=af6604c6-bfb7-5090-b95d-0e2df503d520
    ...
    with parallel():
        # unilab:node_uuid=f8adba77-63c5-5b46-afac-71e908b7ebea
        with group(name="s2 点样 → s4 展开前拍照"):
            # unilab:node_uuid=a3b4b72c-a32a-5332-832a-13e6cb908e30
            spotting = sampling.sampling_execute(  # noqa: F841
                plate_spec=plate_spec,
                plate_no=plate_no,
                well=well,
            )
            # unilab:node_uuid=dce51443-96e8-5573-8040-c44c3efe2d14
            ...
            # unilab:node_uuid=75979140-6056-5a39-a16a-544740232122
            ...
            # unilab:node_uuid=2d7596ba-229c-58c2-b232-3022fe7cb95d
            before_photo = photoscrape.photoscrape_before_photo_capture(  # noqa: F841
                sample_id=sample_id,
                save_dir=save_dir,
            )

        # unilab:node_uuid=274cb2fe-a6a8-534f-8f9d-8270454e8a22
        with group(name="s3 展缸预备"):
            # unilab:node_uuid=ecc6799c-244d-541e-9edc-399dd7f08cd8
            tank_ready = develop.develop_prepare(  # noqa: F841
                tank=tank,
            )

    # Join(s4, s3) -> s5.  The call below cannot start until both branches end.
    # unilab:node_uuid=d3b58ead-dbe7-5b27-b55a-a54568e36fbf
    with group(name="s5 取板进缸"):
        # unilab:node_uuid=7ae98e6e-01ee-50db-89a2-b6f9e6786f10
    ...
    with parallel():
        # unilab:node_uuid=29cd8c58-b38e-5f95-9bc7-e05923cddaab
        with group(name="s6 展开等待 → s8 出缸上刮板台"):
            # unilab:node_uuid=edc228e6-8f22-59be-87be-aee4bb52ba79
            developed = develop.pf_s6_develop_wait(  # noqa: F841
                tank=tank,
                auto_drain=auto_drain,
                dry_duration_s=dry_duration_s,
            )
            # s8 depends only on s6 within this branch, just like parallel_v1.
            # unilab:node_uuid=ff039f9d-ac56-5d84-b8af-0d115742e9c8
    ...
    # unilab:node_uuid=9371c629-c57a-5148-bcbc-b2f4ba92d828
    with group(name="s9 拍照刮取"):
        # unilab:node_uuid=ae450f82-ec40-517b-92fe-c26feacbb0e8
        scraped = photoscrape.photoscrape_process(  # noqa: F841
            sample_id=sample_id,
            save_dir=save_dir,
    ...
    # unilab:node_uuid=bfd6df69-220c-58c4-83c2-9920242b22f0
    with group(name="s10 粉末收集"):
        ...
        with parallel():
            # unilab:node_uuid=d0b543b8-6d1d-59b1-b3c0-8aeefd0d177b
            with group(name="接粉器入收集站"):
                # unilab:node_uuid=21749737-e54f-55e3-a11b-7d950c03c836
            ...
            # unilab:node_uuid=f77e40a4-cc9b-5c9c-b89f-fa2026801d47
            with group(name="收集瓶入收集站"):
                # unilab:node_uuid=36027af7-c4cc-5a6f-b87f-84a09636409d
        ...
        # unilab:node_uuid=223ed167-67df-532a-b29e-8c4dfb2f4aaa
        collection = collect.collect_execute(  # noqa: F841
            solvent_volume_ml=solvent_volume_ml,
            liquid_repeat_count=liquid_repeat_count,
        )
        # unilab:node_uuid=aa17af8a-045e-5fb1-bab0-ad72c68f165d
        lineage = material.record_collection_v4(
            powder_collector=collector_at_collect.resource,
            vial=vial_at_collect.resource,
        )
        with parallel():
            # unilab:node_uuid=417d5b7f-73cb-5a0d-a0c8-ad36310b0fd9
            with group(name="接粉器回中转 A"):
                # unilab:node_uuid=6f389e89-68f8-565a-afd8-dd05b4aa5dcc
            ...
            # unilab:node_uuid=5b54e7bb-4c31-5b85-aad6-21dc605eeb93
            with group(name="收集瓶回中转 B"):
                # unilab:node_uuid=002889a4-e679-5ace-954d-b05e059ae4c2
        ...
        with parallel():
            # unilab:node_uuid=bde51a90-80f1-5136-8217-1a961615daa7
            with group(name="释放中转 A"):
                # unilab:node_uuid=347de1c4-4c94-504e-93d7-5d27f96d887b
                locator_a_released = staging.locator_a(target=False)  # noqa: F841
            # unilab:node_uuid=f9d5e9de-d480-5dcf-8780-3096569ef6f9
            with group(name="释放中转 B"):
                # unilab:node_uuid=f6039ec6-7f8e-524f-aa6d-cd064bb6bba6
                locator_b_released = staging.locator_b(target=False)  # noqa: F841

    # unilab:node_uuid=cff266ff-2b0c-512e-acb8-55a07a96d9fe
    with group(name="s11 废板下料"):
        # unilab:node_uuid=ed593aaa-a60d-58dd-9677-77e36e7531c5
